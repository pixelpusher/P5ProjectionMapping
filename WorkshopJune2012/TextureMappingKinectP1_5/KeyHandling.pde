...
void keyPressed()
{
  if (key == CODED) {
    ...
    if (keyCode == SHIFT)
    {
        bwDynamicImage.zdist = mouseX;
    }
    
    else if (keyCode == UP) {
      bwDynamicImage.deg++;
      bwDynamicImage.deg = constrain(bwDynamicImage.deg, 0, 30);
      bwDynamicImage.kinect.tilt(bwDynamicImage.deg);
    } 
      ...
      bwDynamicImage.thresh--;
      bwDynamicImage.thresh = constrain(bwDynamicImage.thresh, 0, 255);
      println("thresh:" + bwDynamicImage.thresh);
    }
